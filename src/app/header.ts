...
@Component({
  ...
  template: `
    <header class="border-b border-white/5 bg-slate-950/80 backdrop-blur sticky top-0 z-40 px-4 sm:px-6 py-3 sm:py-3.5 flex flex-col md:grid md:grid-cols-3 items-center justify-between gap-3">
      
      <!-- Mobile Layout: Row 1 on Mobile / Left on Desktop: Logo + Title -->
      <div class="flex items-center justify-center md:justify-start gap-2.5 shrink-0 select-none w-full md:w-auto md:justify-self-start">
      ...
      <!-- Mobile Layout: Row 2 on Mobile / Center on Desktop: Model Switch Toggle -->
      ...
      <!-- Mobile Layout: Row 3 on Mobile / Right side on Desktop: Actions & Status Badge -->
      <div class="flex items-center justify-center md:justify-end md:justify-self-end gap-2 text-xs shrink-0 w-full md:w-auto">
        @if (isScriptLoaded()) {
